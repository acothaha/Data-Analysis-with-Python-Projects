...
def calculate_demographic_data(print_data=True):
    ...
    min_work_hours_worker = df.loc[df['hours-per-week'] == min_work_hours].shape[0]

    rich_percentage = round((df.loc[(df['hours-per-week'] == min_work_hours) & (df['salary'] == '>50K')].shape[0] / min_work_hours_worker)*100)

    # What country has the highest percentage of people that earn >50K?

    df['more_than_50K'] = df['salary'] == '>50K'

    result_array = df.groupby('native-country').mean()['more_than_50K']*100

    highest_earning_country = result_array.idxmax()

    highest_earning_country_percentage = round(result_array.max(), 1)
